let additionalQuestions = [
  {
    question: "How does Sorrieu's print identify distinct nations?",
    options: [
      "Through military uniforms",
      "By architectural styles",
      "Using flags and national costumes",
      "Via language inscriptions"
    ],
    answer: 2
  },
  {
    question: "What does the statue of Liberty hold in Sorrieu's print?",
    options: [
      "A crown and scepter",
      "Tricolour flag and constitution",
      "Torch of Enlightenment and Charter of Rights",
      "Broken chains and scales of justice"
    ],
    answer: 2
  },
  {
    question: "Which country immediately follows Germany in Sorrieu's procession?",
    options: [
      "England",
      "Poland",
      "Austria",
      "Russia"
    ],
    answer: 2
  },
  {
    question: "What did Renan specifically criticize as bases for nationhood?",
    options: [
      "Shared struggles and heroes",
      "Common language or territory",
      "Democratic constitutions",
      "Economic systems"
    ],
    answer: 1
  },
  {
    question: "What was renamed the National Assembly during the French Revolution?",
    options: [
      "The Jacobin Club",
      "The Royal Court",
      "The Estates General",
      "The Parlement"
    ],
    answer: 2
  },
  {
    question: "Which practice was discouraged to create French national identity?",
    options: [
      "Commemorating martyrs",
      "Regional dialects",
      "Oath-taking ceremonies",
      "Tricolour display"
    ],
    answer: 1
  },
  {
    question: "What did the French armies carry abroad during revolutionary wars?",
    options: [
      "Napoleonic Code",
      "Ideas of nationalism",
      "Religious relics",
      "Colonial goods"
    ],
    answer: 1
  },
  {
    question: "What does Fig. 2's slogan 'The people must seize their own freedom!' advocate?",
    options: [
      "Monarchical restoration",
      "Foreign intervention",
      "Popular revolution",
      "Religious revival"
    ],
    answer: 2
  },
  {
    question: "Which reform was NOT part of Napoleon's administrative changes?",
    options: [
      "Abolishing feudal system",
      "Introducing regional currencies",
      "Simplifying administrative divisions",
      "Removing guild restrictions"
    ],
    answer: 1
  },
  {
    question: "Who benefited from being freed from serfdom under Napoleon?",
    options: [
      "Guild masters",
      "New businessmen",
      "Peasants",
      "Clergy"
    ],
    answer: 2
  },
  {
    question: "What does Fig. 4's portrayal of French soldiers contradict?",
    options: [
      "Their military discipline",
      "Their revolutionary ideals",
      "Their uniform colors",
      "Their logistical efficiency"
    ],
    answer: 1
  },
  {
    question: "What facilitated business under French administration?",
    options: [
      "Regional trade barriers",
      "Diverse legal systems",
      "Common national currency",
      "Feudal dues"
    ],
    answer: 2
  },
  {
    question: "Where was Napoleon portrayed as a postman in Fig. 5?",
    options: [
      "Returning from Waterloo",
      "After battle of Leipzig",
      "During retreat from Moscow",
      "After Congress of Vienna"
    ],
    answer: 1
  },
  {
    question: "What was the 'daily plebiscite' according to Renan?",
    options: [
      "Regular taxation",
      "Ongoing consent to nationhood",
      "Military conscription",
      "Territorial expansion"
    ],
    answer: 1
  },
  {
    question: "Which group is NOT mentioned in Sorrieu's procession?",
    options: [
      "Kingdom of Two Sicilies",
      "Lombardy",
      "Ottoman Empire",
      "Hungary"
    ],
    answer: 2
  },
  {
    question: "What replaced multinational dynastic empires?",
    options: [
      "Colonial administrations",
      "Theocratic states",
      "Nation-states",
      "Feudal kingdoms"
    ],
    answer: 2
  },
  {
    question: "What did the French Revolutionaries commemorate in the nation's name?",
    options: [
      "Royal coronations",
      "Military defeats",
      "Martyrs",
      "Colonial conquests"
    ],
    answer: 2
  },
  {
    question: "Which city was NOT mentioned as welcoming French armies?",
    options: [
      "Mainz",
      "Brussels",
      "Vienna",
      "Milan"
    ],
    answer: 2
  },
  {
    question: "What did increased taxation and conscription lead to?",
    options: [
      "Economic prosperity",
      "Political freedom",
      "Popular hostility",
      "Cultural exchange"
    ],
    answer: 2
  },
  {
    question: "What did the Napoleonic Code secure?",
    options: [
      "Privileges by birth",
      "Right to property",
      "Manorial dues",
      "Guild monopolies"
    ],
    answer: 1
  }
];
let questions = [...additionalQuestions,
  {
    question: "What does the shattered remains in Sorrieu's print symbolize?",
    options: [
      "Feudal system artifacts",
      "Symbols of absolutist institutions",
      "Ruins of ancient democracies",
      "Destroyed revolutionary flags"
    ],
    answer: 1
  },
  {
    question: "Which nations lead the procession in Sorrieu's 1848 print?",
    options: [
      "France and England",
      "Germany and Austria",
      "United States and Switzerland",
      "Russia and Poland"
    ],
    answer: 2
  },
  {
    question: "What did the German flag in Sorrieu's print represent in 1848?",
    options: [
      "Existing unified nation-state",
      "Colonial ambitions",
      "Liberal hopes for unification",
      "Allegiance to French ideals"
    ],
    answer: 2
  },
  {
    question: "According to Ernst Renan, what forms the basis of a nation?",
    options: [
      "Common language and territory",
      "Shared racial characteristics",
      "Long past of endeavours and sacrifice",
      "Uniform religious practices"
    ],
    answer: 2
  },
  {
    question: "What concept did Renan describe as 'a daily plebiscite'?",
    options: [
      "Democratic elections",
      "Territorial expansion",
      "Existence of nations",
      "Revolutionary struggle"
    ],
    answer: 2
  },
  {
    question: "Which event marked the first clear expression of nationalism?",
    options: [
      "Congress of Vienna (1815)",
      "Napoleonic Wars (1803-1815)",
      "French Revolution (1789)",
      "German unification (1871)"
    ],
    answer: 2
  },
  {
    question: "What did the French revolutionaries replace the royal standard with?",
    options: [
      "Imperial eagle",
      "Tree of Liberty",
      "Tricolour flag",
      "Bastille image"
    ],
    answer: 2
  },
  {
    question: "Which administrative change was NOT introduced by French revolutionaries?",
    options: [
      "Uniform weights and measures",
      "Centralized administration",
      "Regional tax exemptions",
      "Abolition of internal customs"
    ],
    answer: 2
  },
  {
    question: "What was the French revolutionary mission toward Europe?",
    options: [
      "Establish colonial rule",
      "Spread Catholic faith",
      "Liberate peoples from despotism",
      "Enforce French language"
    ],
    answer: 2
  },
  {
    question: "What did Andreas Rebmann's almanac (Fig. 2) juxtapose with Bastille?",
    options: [
      "British naval ship",
      "Prussian military academy",
      "Despotic fortress in Kassel",
      "Vienna Congress hall"
    ],
    answer: 2
  },
  {
    question: "Which principle was NOT part of Napoleonic Code?",
    options: [
      "Equality before law",
      "Privileges based on birth",
      "Right to property",
      "Secular legislation"
    ],
    answer: 1
  },
  {
    question: "Where did Napoleon NOT implement his administrative reforms?",
    options: [
      "Dutch Republic",
      "Ottoman Empire",
      "Switzerland",
      "Italian states"
    ],
    answer: 1
  },
  {
    question: "How does Fig. 4 portray French soldiers in Zweibrücken?",
    options: [
      "Liberators bringing equality",
      "Cultural ambassadors",
      "Oppressors and harassers",
      "Economic reformers"
    ],
    answer: 2
  },
  {
    question: "What economic benefit did businessmen appreciate under Napoleon?",
    options: [
      "Increased guild protections",
      "Regional trade barriers",
      "Uniform laws and currency",
      "Feudal dues abolition"
    ],
    answer: 2
  },
  {
    question: "Where were French armies initially welcomed?",
    options: [
      "Zweibrücken and Leipzig",
      "Madrid and Lisbon",
      "Vienna and Berlin",
      "Brussels and Milan"
    ],
    answer: 3
  },
  {
    question: "What caused hostility toward French rule?",
    options: [
      "Introduction of democracy",
      "Preservation of local traditions",
      "Forced conscription and censorship",
      "Decentralized administration"
    ],
    answer: 2
  },
  {
    question: "What does Fig. 5 symbolize about Napoleon?",
    options: [
      "Military victories",
      "Administrative efficiency",
      "Loss of territories",
      "Cultural influence"
    ],
    answer: 2
  },
  {
    question: "How does the text define 'utopian'?",
    options: [
      "Practical governance model",
      "Imperial expansion strategy",
      "Ideal society unlikely to exist",
      "Revolutionary battle plan"
    ],
    answer: 2
  },
  {
    question: "What did Christ and angels symbolize in Sorrieu's print?",
    options: [
      "Religious authority over states",
      "Divine punishment for revolutionaries",
      "Fraternity among nations",
      "Monarchical legitimacy"
    ],
    answer: 2
  },
  {
    question: "What replaced multi-national dynastic empires in 19th-century Europe?",
    options: [
      "Colonial systems",
      "Theocratic states",
      "Feudal principalities",
      "Nation-states"
    ],
    answer: 3
  },
  {
    question: "Which group helped spread revolutionary ideas through Jacobin clubs?",
    options: [
      "Rural peasantry",
      "Urban nobility",
      "Educated middle classes",
      "Clergy members"
    ],
    answer: 2
  },
  {
    question: "What did the French Revolution transfer sovereignty to?",
    options: [
      "Military generals",
      "Body of citizens",
      "Foreign allies",
      "Religious leaders"
    ],
    answer: 1
  },
  {
    question: "Which practice was introduced to create collective French identity?",
    options: [
      "Regional dialects promotion",
      "Feudal dues collection",
      "Commemoration of martyrs",
      "Royal standard restoration"
    ],
    answer: 2
  },
  {
    question: "What did Napoleonic reforms abolish in occupied territories?",
    options: [
      "Centralized administration",
      "Guild restrictions",
      "Standardized currency",
      "Enlightenment ideals"
    ],
    answer: 1
  },
  {
    question: "What does the sarcastic plaque on Tree of Liberty criticize?",
    options: [
      "German principalities",
      "Napoleonic taxation",
      "French liberation claims",
      "British naval blockade"
    ],
    answer: 2
  },
  {
    question: "Which territory was NOT lost by Napoleon after Leipzig (Fig. 5)?",
    options: [
      "Rhineland",
      "Switzerland",
      "Italy",
      "Poland"
    ],
    answer: 1
  },
  {
    question: "What does Renan consider essential for being a people?",
    options: [
      "Common racial purity",
      "Shared religious doctrine",
      "Common glories and will",
      "Geographical isolation"
    ],
    answer: 2
  },
  {
    question: "Which institution was renamed National Assembly?",
    options: [
      "Jacobin Club",
      "Estates General",
      "Royal Court",
      "Bastille Garrison"
    ],
    answer: 1
  },
  {
    question: "What did Sorrieu's procession NOT include?",
    options: [
      "People of all social classes",
      "Distinct national groups",
      "Asian and African nations",
      "Homage to Liberty statue"
    ],
    answer: 2
  },
  {
    question: "What was the Congress of Vienna's outcome?",
    options: [
      "Unified Germany",
      "Dissolved Austria-Hungary",
      "Redrew European map (Fig. 3)",
      "Abolished monarchies"
    ],
    answer: 2
  }
];
